...
class MyHelperProvider extends ServiceProvider
{
    static public function user(){
        $user = Role::join('role_user','role_user.role_id','roles.id')
            ...
            ->select('roles.slug as role','roles.id')->first();
        return $user;
    }
    static public function batchVillage(){
        return UserInfo::join('batch','batch.id','user_infos.batch_id')
            ->join('villages','villages.id','user_infos.village_id')
            ->select('batch.id as batchId','villages.id as villageId')->where('user_id',\Auth::user()->id)->first();
